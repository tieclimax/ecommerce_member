...
@section('title', 'TopShop || All Notifications')
                ...
                <table class="table  table-hover admin-table" id="notification-dataTable">
                    ...
                    <tbody>
                        @foreach (Auth::user()->Notifications as $notification)
                            ...
                            <tr class="@if ($notification->unread()) bg-light
                            border-left-light @else border-left-success @endif">
                                <td scope="row">{{ $loop->index + 1 }}</td>
                                <td>{{ $notification->created_at->format('F d, Y h:i A') }}</td>
                                <td>{{ $notification->data['title'] }}</td>
                                <td>
                                    <a href="{{ route('superadmin.notification', $notification->id) }}"
                                    ...
                                    <form method="POST"
                                        action="{{ route('superadminnotification.delete', $notification->id) }}">
                                        @csrf
                                        @method('delete')
